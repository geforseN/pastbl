...
function openEmoteCollectionsIdb() {
  if (typeof window === "undefined") {
    return Promise.resolve({} as IDBPDatabase<EmoteCollectionsSchema>);
  }
  return openDB<EmoteCollectionsSchema>("emote-collections", 4, {
...
export async function loadMissingGlobalCollections(
  state: Partial<IGlobalEmoteCollectionRecord>,
) {
  ...
  const idbOfCollections = await collectionsIdb;
  await Promise.all(collections.map(idbOfCollections.global.add));
  return collections;
}

export async function refreshGlobalCollection(
  source: IGlobalEmoteCollection["source"],
) {
  const newCollection = await getGlobalCollection(source);
  await (await collectionsIdb).global.put(newCollection);
...
export async function getAllGlobalCollections() {
  ...
  const collections = await (await collectionsIdb).global.getAll();
  return groupBy(
    collections,
