<template>
  <form class="rounded-box border-2 p-2" @submit.prevent="handleCollectionLoad">
    ...
    <person-emote-collection-fetch-channels-search
...
const mustSelectCollectionOnLoad = useIndexedDBKeyValue(
  "fetch-person-emote-collection:must-select-onload",
  true,
);
...
async function handleCollectionLoad() {
  ...
  await loadCollection(
    async () => {
      ...
      const collection_ = await personsEmoteCollections.loadCollection(login);
